...
public class ParticleHelper {
    public static void spawnItemParticles(Level worldIn, double vectorMul, float height, BlockPos pos, ItemStack stack, int count) {
    ...
    public static void spawnParticles(SimpleParticleType type, Level worldIn, float height, BlockPos pos, int count) {
        for (int i = 0; i < count; ++i) {
            Vec3 vec3d = new Vec3(((double) worldIn.random.nextFloat() - 0.5D) * 0.1D
                    , Math.random() * 0.2D + 0.1D, ((double) worldIn.random.nextFloat() - 0.5D) * 0.1D);
            if (worldIn instanceof ServerLevel) {
                ((ServerLevel) worldIn).sendParticles(type, pos.getX() + 0.5F, pos.getY() + height, pos.getZ() + 0.5F, 1, vec3d.x, vec3d.y + 0.5D, vec3d.z, 0.0D);
            } else {
                worldIn.addParticle(type, pos.getX() + 0.5F, pos.getY() + height, pos.getZ() + 0.5F, vec3d.x, vec3d.y +  0.5D, vec3d.z);
            }
        }
    }
}
