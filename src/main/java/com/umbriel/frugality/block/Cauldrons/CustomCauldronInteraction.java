...
public interface CustomCauldronInteraction {
    ...
    Map<Item, CustomCauldronInteraction> WATER = newInteractionMap();
    Map<Item, CustomCauldronInteraction> LAVA = newInteractionMap();
    Map<Item, CustomCauldronInteraction> POWDER_SNOW = newInteractionMap();
    CustomCauldronInteraction FILL_WATER = (p_175683_, p_175684_, p_175685_, p_175686_, p_175687_, p_175688_) -> {
        return emptyBucket(p_175684_, p_175685_, p_175686_, p_175687_, p_175688_, getWaterCauldron(p_175683_).setValue(CustomLayeredCauldron.LEVEL, Integer.valueOf(3)), SoundEvents.BUCKET_EMPTY);
    };
    CustomCauldronInteraction FILL_LAVA = (p_175676_, p_175677_, p_175678_, p_175679_, p_175680_, p_175681_) -> {
        return emptyBucket(p_175677_, p_175678_, p_175679_, p_175680_, p_175681_, getLavaCauldron(p_175676_), SoundEvents.BUCKET_EMPTY_LAVA);
    };
    ...
    CustomCauldronInteraction SHULKER_BOX = (p_175662_, p_175663_, p_175664_, p_175665_, p_175666_, p_175667_) -> {
        ...
        if (!(block instanceof ShulkerBoxBlock)) {
            ...
            if (!p_175663_.isClientSide) {
                ...
                CustomLayeredCauldron.lowerFillLevel(p_175662_, p_175663_, p_175664_);
            }

            return InteractionResult.sidedSuccess(p_175663_.isClientSide);
        }
    };
    CustomCauldronInteraction BANNER = (p_175653_, p_175654_, p_175655_, p_175656_, p_175657_, p_175658_) -> {
        if (BannerBlockEntity.getPatternCount(p_175658_) <= 0) {
            ...
            if (!p_175654_.isClientSide) {
                ...
                CustomLayeredCauldron.lowerFillLevel(p_175653_, p_175654_, p_175655_);
            }

            return InteractionResult.sidedSuccess(p_175654_.isClientSide);
        }
    };
    CustomCauldronInteraction DYED_ITEM = (p_175629_, p_175630_, p_175631_, p_175632_, p_175633_, p_175634_) -> {
        ...
        if (!(item instanceof DyeableLeatherItem)) {
            ...
            if (!dyeableleatheritem.hasCustomColor(p_175634_)) {
                ...
                if (!p_175630_.isClientSide) {
                    ...
                    CustomLayeredCauldron.lowerFillLevel(p_175629_, p_175630_, p_175631_);
                }
    ...
    static Object2ObjectOpenHashMap<Item, CustomCauldronInteraction> newInteractionMap() {
    ...
    static void bootStrap() {
        ...
        EMPTY.put(Items.POTION, (p_175732_, p_175733_, p_175734_, p_175735_, p_175736_, p_175737_) -> {
            if (PotionUtils.getPotion(p_175737_) != Potions.WATER) {
                ...
                if (!p_175733_.isClientSide) {
                    ...
                    p_175733_.setBlockAndUpdate(p_175734_, getWaterCauldron(p_175732_));
        ...
        WATER.put(Items.BUCKET, (p_175725_, p_175726_, p_175727_, p_175728_, p_175729_, p_175730_) -> {
            return fillBucket(p_175725_, p_175726_, p_175727_, p_175728_, p_175729_, p_175730_, new ItemStack(Items.WATER_BUCKET), (p_175660_) -> {
                return p_175660_.getValue(CustomLayeredCauldron.LEVEL) == 3;
        ...
        WATER.put(Items.GLASS_BOTTLE, (p_175718_, p_175719_, p_175720_, p_175721_, p_175722_, p_175723_) -> {
            if (!p_175719_.isClientSide) {
                ...
                CustomLayeredCauldron.lowerFillLevel(p_175718_, p_175719_, p_175720_);
        ...
        WATER.put(Items.POTION, (p_175704_, p_175705_, p_175706_, p_175707_, p_175708_, p_175709_) -> {
            if (p_175704_.getValue(CustomLayeredCauldron.LEVEL) != 3 && PotionUtils.getPotion(p_175709_) == Potions.WATER) {
                if (!p_175705_.isClientSide) {
                    ...
                    p_175705_.setBlockAndUpdate(p_175706_, p_175704_.cycle(CustomLayeredCauldron.LEVEL));
        ...
        POWDER_SNOW.put(Items.BUCKET, (p_175690_, p_175691_, p_175692_, p_175693_, p_175694_, p_175695_) -> {
            return fillBucket(p_175690_, p_175691_, p_175692_, p_175693_, p_175694_, p_175695_, new ItemStack(Items.POWDER_SNOW_BUCKET), (p_175627_) -> {
                return p_175627_.getValue(CustomLayeredCauldron.LEVEL) == 3;
            }, SoundEvents.BUCKET_FILL_POWDER_SNOW);
        });
        addDefaultInteractions(POWDER_SNOW);
    }

    static void addDefaultInteractions(Map<Item, CustomCauldronInteraction> Map) {
    ...
    static InteractionResult fillBucket(BlockState p_175636_, Level p_175637_, BlockPos p_175638_, Player p_175639_, InteractionHand p_175640_, ItemStack p_175641_, ItemStack p_175642_, Predicate<BlockState> p_175643_, SoundEvent p_175644_) {
        if (!p_175643_.test(p_175636_)) {
            ...
            if (!p_175637_.isClientSide) {
                ...
                p_175637_.setBlockAndUpdate(p_175638_, getCauldron(p_175636_));
                System.out.println(p_175636_.getBlock());
